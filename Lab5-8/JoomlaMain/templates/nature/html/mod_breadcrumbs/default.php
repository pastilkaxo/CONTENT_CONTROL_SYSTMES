...
if (!$attribs && isset($module->npAttrs)) {
    $attribs = json_decode($module->npAttrs, true);
}
$id = $attribs && isset($attribs['id']) ? $attribs['id'] : '';
$style = isset($attribs['style']) ? $attribs['style'] : '';
$modPath = dirname(__FILE__) . '/breadcrumbs/default_breadcrumbs_';
if ($style == 'upstyle' && file_exists($modPath . $id . '.php')) {
    include($modPath . $id . '.php');
} else if ($style == 'upstylefromplugin' && file_exists($modPath . '1.php')) {
    include($modPath . '1.php');
} else {
?>
